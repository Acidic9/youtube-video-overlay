...
function getOptions() {
	chrome.storage.sync.get({
		...
		youtubePreviewOptions = opt;
		youtubePreviewOptions.volume = (!isNaN(youtubePreviewOptions.volume)) ? youtubePreviewOptions.volume : 0;
		youtubePreviewOptions.playbackSpeed = (!isNaN(youtubePreviewOptions.playbackSpeed)) ? youtubePreviewOptions.playbackSpeed : 2;
		youtubePreviewOptions.resumePlayback = (youtubePreviewOptions.resumePlayback) ? true : false;
	});
}
...
// Add the youtube javascript lib
{
...
function ready() {
	// Add watchlater butter and video time above overlay	
	var style = jQuery('<style>.related-list-item .video-actions, .video-time { z-index: 100; }</style>');
	jQuery('html > head').append(style);

	// Add embeded video to each video item
	jQuery(".thumb-wrapper, .yt-thumb").one('mouseenter', function() {
		jQuery(this).find(".video-time").each(function() {
			// Get the video ID
			var videoID = /v=([^"&?\/ ]{11})/g.exec(jQuery(this).closest("a").attr("href"))[1];

			// If invalid video ID, then exit
			if (videoID == "")
				return;

			thumbnails[videoID] = jQuery(this).parent().children("img");
			var overlayW = thumbnails[videoID].width();
			var overlayH = thumbnails[videoID].height();

			jQuery(this).parent().prepend('<span id="video-preview-'+videoID+'" style="display: none"></span>');
			jQuery(this).parent().prepend('<div class="youtube-hover-overlay" id="youtube-overlay-'+videoID+'"></div>');
			players[videoID] = new YT.Player('video-preview-'+videoID, {
				width: overlayW,
				height: overlayH,
				videoId: videoID,
				playerVars: {
				...
				events: {
					'onReady': function(event) {
						...
						player.setPlaybackRate(youtubePreviewOptions.playbackSpeed);
						player.setVolume(youtubePreviewOptions.volume);
						player.setPlaybackQuality("small");

						var videoID = player.getVideoData()['video_id'];
						isReady[videoID] = true;
						if (mouseover == videoID) {
							players[videoID].playVideo();
							jQuery("#video-preview-"+videoID).show();
							thumbnails[videoID].hide();
						}
					}
				}
			});

			jQuery("#youtube-overlay-"+videoID).css({
				"width": "100%",
				"height": "100%",
				"left": "0",
				"position": "absolute",
				...
				var videoID = /youtube-overlay-([^"&?\/ ]{11})/g.exec(jQuery(this).attr("id"))[1];
				mouseover = videoID;
				if (isReady[videoID] !== true)
					return;
				if (!youtubePreviewOptions.resumePlayback)
				...
				jQuery("#video-preview-"+videoID).hide();
				thumbnails[videoID].show();
			});
		});
	});
}